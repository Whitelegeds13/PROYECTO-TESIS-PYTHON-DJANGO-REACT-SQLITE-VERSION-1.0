...
export default function EmployeeLogin({ onLogin, afterLoginPath = '/' }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      const normalized = raw.includes('@') || raw.toUpperCase().startsWith('GMR-') ? raw : `GMR-${raw}`
      await onLogin({ username: normalized, password })
      navigate(afterLoginPath)
    } catch (err) {
      setError(err?.message || 'No se pudo acceder')
    } finally {
      setLoading(false)
    }
  }
          ...
          <div className="w-full max-w-md overflow-hidden rounded-3xl border border-white/10 bg-[#071026]/70 shadow-2xl shadow-black/50 backdrop-blur-2xl">
            ...
            <form onSubmit={submit} className="space-y-4 px-6 pb-6 pt-6">
              ...
              <div className="pt-2 text-center text-sm">
                <Link to="/iniciar-sesion" className="text-white/55 hover:text-white">
                  Volver
                </Link>
              </div>
            </form>
